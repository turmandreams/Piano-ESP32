...
void recogeteclas(){
  ...
  if(playing){
    ...
    if(musicaposicion<musicatam){     
      ...
      if((millis()-tiempomusica)>musicatiempoinicio[musicaposicion]){
    ...
    if(!teclas[i]){  //Tecla no pulsada   
      if((millis()-tiempoteclas[i])>50){         
        ...
        if(numeros[i]>120){
          ...
          if(efectochorus){
            if(i<38){frecuenciasonido[i+1]=(f+5);possonido[i+1]=0; }
            if(i>0){frecuenciasonido[i-1]=(f-5);possonido[i-1]=0; }            
          }
...
boolean pulsacionbotones(){
  ...
  if(digitalRead(15)==LOW){ //Play y Stop Notas guardadas
      if(!grabando){
        ...
        if(playing){
            ...
            drawCentreString("Playing!!",64,32,2); 
            display.display();
  ...
  if(digitalRead(4)==LOW){  ///Boton de Grabacion
      if(!playing){
        if(!grabando){          
            display.clearDisplay();
            drawCentreString("Recording!",64,32,2); 
            display.display();
            tiempoinicio=millis();
            tiempon=0;
            musicatam=0;
