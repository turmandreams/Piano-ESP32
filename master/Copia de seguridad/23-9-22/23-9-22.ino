...
void recogeteclas(){
 ...
 // if((numeros[0]==0)&&(numeros[2]==0)){ cargarsonido[botonseleccionado]=true; }  
  ...
  while(sigue){
    ...
    for(int i=0;i<4;i++){
      ...
      if(sonidorelleno[i]){  
        ...
        if(possonido[i]<(tamsonido[i]-1000)){
          ...
          audioBuffer[j]   +=  dato*volumen;
          audioBuffer[j+1] +=  dato*volumen;
