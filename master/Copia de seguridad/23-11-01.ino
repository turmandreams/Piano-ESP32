...
void cargasonido(int numsonido){
  ...
  if(posarchivos[numsonido]==0){ 
      ...
      for(int i=0;i<39;i++){
        possonido[numsonido][i]=pianotam;        
      }
    ...
    // Serial.print("Se va abrir el archivo : ");Serial.println(nombrearchivo);    
...
void teclasluces(int i){
    if(teclas[i][0]){  led(i,0,true);  }
    if(teclas[i][2]){  led(i,1,true);  }
    if(teclas[i][4]){  led(i,2,true);  }
    if(teclas[i][5]){  led(i,3,true);  }
    if(teclas[i][7]){  led(i,4,true);  }
    if(teclas[i][9]){  led(i,5,true);  }
    if(teclas[i][11]){ led(i,6,true);  }
    
    if(teclas[i][12]){  led(i,7,true);  }
    if(teclas[i][14]){  led(i,8,true);  }
    if(teclas[i][16]){  led(i,9,true);  }
    if(teclas[i][17]){  led(i,10,true);  }
    if(teclas[i][19]){  led(i,11,true);  }
    if(teclas[i][21]){  led(i,12,true);  }
    if(teclas[i][23]){  led(i,13,true);  }
    
    if(teclas[i][24]){  led(i,14,true);  }
    if(teclas[i][26]){  led(i,15,true);  }
    if(teclas[i][28]){  led(i,16,true);  }
    if(teclas[i][29]){  led(i,17,true);  }
    if(teclas[i][31]){  led(i,18,true);  }
    if(teclas[i][33]){  led(i,19,true);  }
    if(teclas[i][35]){  led(i,20,true);  }
    
    if(teclas[i][36]){  led(i,21,true);  }
    if(teclas[i][38]){  led(i,22,true);  }
  
}

void luces(){

  for(int i=0;i<23;i++){ led(0,i,false); }
  
  for(int i=0;i<4;i++){  teclasluces(i);   }

  teclasluces(canalseleccionado);
        ...
        if(!teclas[j][i]){  //Tecla no pulsada   
          if((millis()-tiempoteclas[j][i])>50){         
            ...
            if(numeros[j][i]>120){
              ...
              if(efectochorus[j]){
                if(i<38){frecuenciasonido[j][i+1]=(f+5);possonido[j][i+1]=0; }
                if(i>0){frecuenciasonido[j][i-1]=(f-5);possonido[j][i-1]=0; }  
                
                //if(i<38){frecuenciasonido[j][i+1]=(f+0.2);possonido[j][i+1]=0; }
                //if(i>0){frecuenciasonido[j][i-1]=(f-0.2);possonido[j][i-1]=0; }              
              }
  ...
  valor=analogRead(34);if(valor<255){  //Boton Abajo
     if(menuplaying){
       ...
       if(menu==0){
          ...
          efectochorus[canalseleccionado]=!efectochorus[canalseleccionado];
          menupantalla();  
       }       
     }
      ...
      if(posarchivos[i]!=0){  
        ...
        for(int jj=0;jj<39;jj++){
          ...
          if(possonido[i][jj]<tam){
            ...
            else { dato1 = sonido[i][ii+1]<<8 | sonido[i][ii]; }

            int16_t dato2;      
            if(posarchivos[i]==1){ dato2 = piano[ii+3]<<8 | piano[ii+2]; }
            else { dato2 = sonido[i][ii+3]<<8 | sonido[i][ii+2]; }
...
void menupantalla(){
  ...
  if(menu==0){
    ...
    String texto="ON";
    if(!efectochorus[canalseleccionado]){ texto="OFF"; }
    drawCentreString(texto,64,52,1);    
  }
...
void definecolor(int j,int colordefinido){
  ...
  else if(colordefinido==4){ rcolor[j]=200;gcolor[j]=50;bcolor[j]=50;}
